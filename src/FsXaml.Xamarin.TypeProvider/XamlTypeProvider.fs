...
module XamlTypeUtils =
    let internal xfAssembly = typeof<Xamarin.Forms.Button>.Assembly
    ...
    let internal xamlTypes = 
        let assem = typeof<Xamarin.Forms.Button>.GetTypeInfo().Assembly
        let types = assem.GetTypes()
        types
        |> Seq.filter (fun t -> t.Namespace = "Xamarin.Forms")
        |> Seq.toArray

    let internal createXamlNode filename isRoot (xaml:XmlReader) (rootNodeType : RootNodeType option) =
        ...
        try 
            ...
            | Some name -> 
                let propertyType =
                    ...
                    | "Color" -> typeof<Xamarin.Forms.Color>
                    | _ ->
                        let r = xamlTypes 
                        // TODO: Figure out how we handle types, given that we don't have xaml tooling anymore!
                        // Tricky part is going to be handling user defined types - core types (button,etc) should match with this...
                        let xamltype = r |> Seq.tryFind (fun xt -> xt.Name = xaml.LocalName)
                        match xamltype with
                        | None   -> typeof<obj>
                        | Some t -> t.GetTypeInfo().UnderlyingSystemType
                { Position = pos
                  IsRoot = isRoot
                  Name = name
